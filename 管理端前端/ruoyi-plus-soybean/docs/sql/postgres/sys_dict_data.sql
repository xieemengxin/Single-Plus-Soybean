-- ------------------------------------------------------------------------------------------
-- soybean 前端字典适配脚本（RuoYi-Vue-Plus 单体版，PostgreSQL）
--
-- 用途：
--   将 script/sql/postgres/postgres_ry_vue.sql 初始化出来的 sys_dict_type / sys_dict_data
--   种子数据适配为 ruoyi-plus-soybean 前端可识别的形式：
...
--   字典类型保持 postgres_ry_vue.sql 原样（含 sys_user_gender），前端页面与语言包
...
--   2. wf_* 三组字典（业务状态/表单类型/任务状态）来自可选脚本
--      script/sql/postgres/postgres_ry_workflow.sql，未执行该脚本时对应 UPDATE 命中 0 行，无副作用；
--   3. 本脚本中所有 WHERE 的 dict_code 均对准 postgres_ry_vue.sql / postgres_ry_workflow.sql
--      中真实存在的行；
...
-- 以下 wf_* 字典来自可选脚本 postgres_ry_workflow.sql（workflow 菜单虽已停用，字典 i18n 化无副作用，保留适配）
