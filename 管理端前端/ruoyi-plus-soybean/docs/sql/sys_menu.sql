-- ------------------------------------------------------------------------------------------
-- soybean 前端菜单适配脚本（RuoYi-Vue-Plus 单体版）
--
-- 用途：
--   将 script/sql/ry_vue.sql 初始化出来的 sys_menu 种子数据适配为 ruoyi-plus-soybean
--   前端可识别的形式（component 特殊值 Layout/ParentView/FrameView、iconify 图标、
--   i18n 菜单名、iframe 控制台、about 页面、停用前端没有对应页面的菜单）。
--
-- 执行前提：
--   1. 必须先执行 script/sql/ry_vue.sql 完成建库建表和种子数据初始化；
--   2. script/sql/ry_workflow.sql 为可选脚本，本文件末尾的工作流菜单停用语句
--      仅在其已执行时生效（未执行时 UPDATE 命中 0 行，无副作用）；
--   3. 本脚本中所有 UPDATE 的 menu_id 均对准 ry_vue.sql / ry_workflow.sql 中真实存在的行。
--
-- 幂等性：
--   所有 UPDATE 重复执行结果一致；所有 INSERT 均带 ON DUPLICATE KEY UPDATE，可重复执行。
--
-- 其他说明：
...
-- 登录日志：ry_vue.sql 种子为 path='logininfo'、component='monitor/logininfo/index'（无 r），
...
--     menu_id 1761400000000000009 已核实在 ry_vue.sql 等种子中空闲）
-- ----------------------------
...
VALUES (1761400000000000009, 'route.about', 0, 99, 'about', 'about/index', '', 'N', 'Y', 'C', '0', '0', '', 'fluent:book-information-24-regular', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, '关于页面')
...
ON DUPLICATE KEY UPDATE `update_time` = sysdate();
-- 注：Cloud 版脚本此行用的是 1761400000000000008，但单体种子中该 ID 已被「AI会话」占用，
--     故改用已核实空闲的 1761400000000000010
INSERT INTO `sys_menu` (`menu_id`, `menu_name`, `parent_id`, `order_num`, `path`, `component`, `query_param`, `is_frame`, `is_cache`, `menu_type`, `visible`, `status`, `perms`, `icon`, `active_menu`, `ext`, `create_dept`, `create_by`, `create_time`, `update_by`, `update_time`, `remark`)
VALUES (1761400000000000010, 'RuoYi-Plus-Soybean', 0, 100, 'https://gitee.com/xlsea/ruoyi-plus-soybean', 'FrameView', '', 'Y', 'Y', 'M', '0', '0', '', 'local-icon-gitee', '', '', 1761000000000000103, 1761100000000000001, sysdate(), null, null, 'RuoYi-Plus-Soybean 仓库地址')
ON DUPLICATE KEY UPDATE `update_time` = sysdate();

-- ----------------------------
-- 六、停用 soybean 前端没有对应页面的菜单
-- ----------------------------
-- 说明：前端 hasAuth 使用的按钮权限串已与 ry_vue.sql 种子的 F 型按钮行逐一核对一致
...
UPDATE `sys_menu` SET `status` = '1' WHERE `menu_id` = 1761400000000000008;
...
-- 这些行来自可选脚本 script/sql/ry_workflow.sql，未执行该脚本时以下 UPDATE 命中 0 行，无副作用
...
-- 测试菜单下的请假申请（workflow/leave/index，soybean 前端无此视图，同样来自 ry_workflow.sql）
