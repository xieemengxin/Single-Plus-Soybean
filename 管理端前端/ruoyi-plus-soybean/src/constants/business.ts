...
/** menu frame type（表单三态，提交时映射为 isFrame + component） */
export const menuFrameTypeRecord: Record<Api.System.MenuFrameType, string> = {
  external: '是',
  internal: '否',
  iframe: 'iframe'
};

export const menuFrameTypeOptions = transformRecordToOption(menuFrameTypeRecord);
...
/** gen frontend type */
export const genFrontendTypeRecord: Record<string, string> = {
  soybean: 'Soybean（本前端）',
  vue: 'Vue3 + ElementPlus（plus-ui）',
  react: 'React + AntDesign'
};

export const genFrontendTypeOptions = transformRecordToOption(genFrontendTypeRecord);
