...
export function fetchResetUserTotp(userId: CommonType.IdType) {
  return request<boolean>({
    url: `/system/user/resetTotp/${userId}`,
    method: 'put'
  });
}

/** 修改用户头像（先上传 OSS，再把 ossId 写入个人信息） */
export function fetchUpdateUserAvatar(avatar: CommonType.IdType) {
  return request<boolean>({
    url: '/system/user/profile',
    method: 'put',
    data: { avatar }
