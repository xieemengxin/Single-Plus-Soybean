...
const imageUrl = ref(authStore.userInfo.user?.avatarUrl || defaultAvatar);
...
async function handleCrop() {
  ...
  try {
    ...
    // 将 canvas 转换为 blob，先上传到 OSS，再把 ossId 写入个人信息
    canvas.toBlob(async (blob: Blob | null) => {
      if (!blob) return;

      const file = new File([blob], options.fileName || 'avatar.png', { type: 'image/png' });

      const { data: oss, error: uploadError } = await fetchUploadFile(file);
      if (uploadError) return;

      const { error } = await fetchUpdateUserAvatar(oss.ossId);
      if (!error) {
        window.$message?.success('头像更新成功！');
        imageUrl.value = oss.url;
        authStore.userInfo.user!.avatarUrl = oss.url;
        hideDrawer();
      }
    }, 'image/png');
  } finally {
