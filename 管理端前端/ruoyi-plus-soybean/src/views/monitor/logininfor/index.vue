...
async function handleExport() {
  download('/monitor/loginInfo/export', searchParams.value, `登录日志记录_${new Date().getTime()}.xlsx`);
}
    ...
    <NCard title="登录日志列表" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
      <template #header-extra>
        <TableHeaderOperation
          ...
          :loading="loading"
          :show-add="false"
          :show-delete="hasAuth('monitor:logininfo:remove')"
          :show-export="hasAuth('monitor:logininfo:export')"
        ...
        >
          <template #prefix>
            <NButton
              v-if="hasAuth('monitor:logininfo:remove')"
              type="error"
              ghost
              size="small"
