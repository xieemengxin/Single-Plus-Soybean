...
// 外链类型
const isExternalType = computed(() => currentMenu.value?.isFrame === 'Y');

// iframe类型（isFrame=N 且组件为 FrameView）
const isIframeType = computed(() => currentMenu.value?.isFrame === 'N' && currentMenu.value?.component === 'FrameView');
          ...
          >
            ...
            <NDescriptionsItem v-if="isMenu" :label="$t('page.system.menu.isCache')">
              <NTag v-if="currentMenu.isCache" class="m-1" size="small" :type="tagMap[currentMenu.isCache]">
                {{ currentMenu.isCache === 'Y' ? $t('page.system.menu.cache') : $t('page.system.menu.noCache') }}
              </NTag>
            </NDescriptionsItem>
          </NDescriptions>
        </NCard>
