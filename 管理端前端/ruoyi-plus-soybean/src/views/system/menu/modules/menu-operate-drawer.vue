...
import { menuFrameTypeOptions, menuIconTypeOptions, menuLayoutOptions, menuTypeOptions } from '@/constants/business';
...
/** 表单三态：内部路由 / 外链 / iframe，提交时映射为 isFrame + component + queryParam */
const frameType = ref<Api.System.MenuFrameType>('internal');
...
// 外链类型
const isExternalType = computed(() => frameType.value === 'external');

// 内部类型
const isInternalType = computed(() => frameType.value === 'internal');

// 空白布局
const isBlankLayout = computed(() => layoutType.value === '1');

// iframe类型
const isIframeType = computed(() => frameType.value === 'iframe');
...
function handleInitModel() {
  ...
  iconType.value = '1';
  layoutType.value = '0';
  frameType.value = 'internal';
  model.value = createDefaultModel();

  if (props.operateType === 'edit' && props.rowData) {
    Object.assign(model.value, jsonClone(props.rowData));

    // 由后端字段还原表单三态：Y=外链；N + FrameView=iframe；其余为内部路由
    if (model.value.isFrame === 'Y') {
      frameType.value = 'external';
    } else if (model.value.component === 'FrameView') {
      frameType.value = 'iframe';
    } else {
      frameType.value = 'internal';
    }
...
async function handleSubmit() {
  ...
  const payload = {
    ...
    isFrame: frameType.value === 'external' ? 'Y' : ('N' as Api.System.IsMenuFrame),
    isCache,
    menuType,
    visible: menuVisible,
    status,
...
watch(
  ...
  newType => {
    if (newType === 'M' && frameType.value === 'iframe') {
      frameType.value = 'internal';
    }
  }
);
      ...
      <NForm ref="formRef" :model="model" :rules="rules">
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi v-if="!isBtn" :span="12" path="isFrame">
            ...
            <NRadioGroup v-model:value="frameType">
              <NSpace>
                <NRadio
                  v-for="option in menuFrameTypeOptions"
                  :key="option.value"
                  :value="option.value"
                  :label="option.label"
                  :disabled="option.value === 'iframe' && isCatalog"
                />
              </NSpace>
            </NRadioGroup>
          </NFormItemGi>
