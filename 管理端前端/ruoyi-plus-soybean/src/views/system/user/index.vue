  ...
  useNaivePaginatedTable({
    ...
    columns: () => [
        ...
        render: row => {
          return (
            <div class="flex items-center justify-center gap-2">
              <NAvatar src={row.avatarUrl} class="bg-primary">
                {row.avatarUrl ? undefined : row.nickName.charAt(0)}
              </NAvatar>
      ...
      },
      {
        ...
        ellipsis: true,
        render(row) {
          return <DictTag value={row.gender} dictCode="sys_user_gender" />;
        }
      },
      {
        key: 'deptName',
      ...
      },
      {
        ...
        render: row => {
          ...
          const totpBtn = () => {
            return (
              <ButtonIcon
                text
                type="warning"
                icon="material-symbols:phonelink-lock-outline"
                tooltipContent="重置两步验证"
                popconfirmContent={`确认重置 ${row.userName} 的两步验证绑定吗？`}
                onPositiveClick={() => handleResetTotp(row.userId)}
              />
            );
          };
          ...
          if (hasAuth('system:user:resetPwd')) buttons.push(totpBtn());
...
/** 重置用户两步验证绑定，重置后该用户下次登录需重新绑定认证器 */
async function handleResetTotp(userId: CommonType.IdType) {
  const { error } = await fetchResetUserTotp(userId);
  if (error) return;
  window.$message?.success('两步验证已重置');
}
