...
const tab = ref('fm/java/domain.java.ftl');
...
watch(visible, () => {
  if (visible.value) {
    previewData.value = {};
    tab.value = 'fm/java/domain.java.ftl';
    getGenPreview();
  }
});

/** 后端预览接口的 key 为 FreeMarker 模板路径（如 fm/soybean/api.ts.ftl），取文件名去掉 .ftl 后缀作为页签名 */
function getTabLabel(templatePath: string) {
  return templatePath.split('/').pop()!.replace(/\.ftl$/, '');
}
    ...
    <NDrawerContent title="代码预览" :native-scrollbar="false" closable>
      <NSpin :show="loading" class="h-full" content-class="h-full">
        <div class="flex flex-col">
          ...
          <div v-if="tab.endsWith('.vue.ftl')" class="flex p-16px">
          ...
          <NCode
            ...
            :language="getGenLanguage(getTabLabel(tab))"
            show-line-numbers
          />
